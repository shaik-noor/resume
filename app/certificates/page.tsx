  ...
  return (
    <section className="mx-auto max-w-5xl space-y-8">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Selected certifications and trainings.</p>
      </header>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {informaticaCertificates.map((c) => (
            <div key={c.title} className="rounded-lg border p-0 overflow-hidden">
              <div className="aspect-video w-full bg-muted">
                {/* Use plain img to avoid remote domain config requirements */}
                <img
                  src={c.image}
                  alt={c.title}
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {c.skills.map((s) => (
                    <span key={s} className="inline-flex items-center rounded-full border px-2 py-0.5 text-[11px]">
                      {s}
                    </span>
                  ))}
                </div>
